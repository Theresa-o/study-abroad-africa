...
import { usePOF } from "@/app/hooks/proofOfFunds/usePOF";
...
const validationSchema = Yup.object({
  ...
  pof_interest: Yup.string().required("Tell us your background"),
});
...
const POFInquiry = () => {
  ...
  const handleSubmit = async (
    ...
    const { first_name, last_name, email, phone_no, pof_interest } = values;
      ...
      {({ errors, touched, isSubmitting, setFieldValue, values }) => (
        <Form className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden my-8">
          ...
          <div className="px-6 py-8 space-y-6">
            ...
            <div>
              <label
                htmlFor="pof_interest"
                className="block text-sm font-medium text-gray-700 mb-1"
              >
                Please give us relevant information on your proof of funds
                interest and amount
              </label>
              <TiptapEditor
                content={values.pof_interest}
                onChange={(content) => setFieldValue("pof_interest", content)}
                error={!!(errors.pof_interest && touched.pof_interest)}
              />
              <ErrorMessage
                name="pof_interest"
                component="div"
